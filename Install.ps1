param (
  [Parameter()]
  [switch]
  $UninstallSpotifyStoreEdition = (Read-Host -Prompt 'Desinstalar version de spotify de la Windows Store si existe? (Y/N)') -eq 'y',
  [Parameter()]
  [switch]
  $UpdateSpotify,
  [Parameter()]
  [switch]
  $RemoveAdPlaceholder = (Read-Host -Prompt 'Quitar botones y banners extras? (Y/N)') -eq 'y'
)
...
function Get-File
{
  ...
  else
  {
    ...
    while ($count -gt 0)
    {
      ...
      Write-Progress -Activity "Descargando archivo '$downloadedFileName'" -Status "Downloaded ($([System.Math]::Floor($downloadedBytes/1024))K of $($totalLength)K): " -PercentComplete ((([System.Math]::Floor($downloadedBytes / 1024)) / $totalLength) * 100)
    }

    Write-Progress -Activity "Archivo descargado: '$downloadedFileName'"
...
{
  Write-Host "Se ha encontrado la versión de Windows Store de Spotify.`n"
  ...
  {
    Read-Host "Saliendo...`nPress any key to exit..."
    exit
  }
}
...
Write-Host "Descargando ultima versión del parche (chrome_elf.zip)...`n"
$elfPath = Join-Path -Path $PWD -ChildPath 'chrome_elf.zip'
try
{
  $uri = 'https://github.com/holasoyender/SpotifyBlocker/releases/latest/download/chrome_elf.zip'
...
{
  if ((Read-Host -Prompt 'Para instalar está versión debes de tener la versión más reciente de Spotify, quieres continuar? (Y/N)') -ne 'y')
  {
    exit
  }
}
...
{
  Write-Host 'Descargando la ultima versión de Spotify full setup, por favor espera...'
...
if ($RemoveAdPlaceholder)
{
  ...
  elseif (Test-Path $xpuiUnpackedPath)
  {
    ...
    Write-Host 'Se ha detectado Spicetify - Necesitarás reinstalarlo una vez acabae.';
  }
  else
  {
    Write-Host 'No se ha encontrado xpui.js, por favor, abre una issue en el repositorio.'
  }
...
{
  Write-Host "No se modificaran los botones y banners opcionales.`n"
}
...
Write-Host 'Spotify Blocker completado!, iniciando Spotify...'
